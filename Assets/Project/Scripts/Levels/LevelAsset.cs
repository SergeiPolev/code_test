...
namespace Levels
{
    ...
    public class LevelAsset : ScriptableObject
    {
        ...
        [Header("Стопки (стартовые)")]
        public List<StackDef> stacks = new();

        [Header("Заблокированные клетки")]
        public List<CellRef> blockedCells = new();
        
        // ------------- ЦЕЛЬ УРОВНЯ: СЖИГАНИЕ ----------------

        [Header("Burn Goal")]
        [Tooltip("Сколько гексов нужно сжечь, чтобы пройти уровень")]
        ...
        public bool IsColorAvailable(ColorID color, int burnedCount)
        {
            ...
            {
                if (rule.color != color) continue;
                hasRule = true;
                if (rule.unlockAtBurned < minUnlock)
                    minUnlock = rule.unlockAtBurned;
            }

            if (!hasRule) return false; // нет правил — цвет не используется на этом уровне
